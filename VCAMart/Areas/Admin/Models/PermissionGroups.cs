...
namespace RICTotalAdmin.Models
{
    public class PermissionGroups
    {
        ...
        {
            string sql = "exec Admin.sp_" + tableName + "_Get_List ";
           return common.RunSQLToJson(sql, ref msg);
        }

        public string GetPhanQuyen(string nq)
        {
            string sql = "exec Admin.sp_" + tableName + "_PhanQuyen_GetList '" + nq +"'";
            return common.RunSQLToJson(sql, ref msg);
        }

        public bool Insert(dynamic row)
        {
            sql = "EXEC Admin.sp_" + tableName + "_Insert N'" + row.id.ToString() + "',N'" + row.name.ToString() + "', " + row.active.ToString() + ",N'" + User_ID.ToString() + "'";
        ...
        {
            sql = "EXEC Admin.sp_" + tableName + "_Update N'" + row.id.ToString() + "',N'" + row.name.ToString() + "', " + row.active.ToString() + ",N'" + RC_ID.ToString() + "',N'" + User_ID.ToString() + "'";
            RICDB.DB.RunSQLNoReturn(sql, ref msg, strcnn);
            return (msg == "100" ? true : false);
        }
        public bool Delete(string ids)
        {
            sql = "EXEC Admin.sp_" + tableName + "_Delete N'" + ids + "',N'" + RC_ID.ToString() + "',N'" + User_ID.ToString() + "'";
            RICDB.DB.RunSQLNoReturn(sql, ref msg, strcnn);
        ...
        public bool Import(dynamic arr, dynamic cols)
        {
            ...
            sql = "EXEC Admin.sp_" + tableName + "_Import N'" + tableName + "',N'" + RC_ID.ToString() + "',N'" + User_ID.ToString() + "'";
            common.RunSQLNoReturn(sql, ref msg, strcnn, ref iRowEffected);
            return (msg == "100" ? true : false);
        }
        ...
        public bool SavePhanQuyen(dynamic data)
        {
            ...
            string sql = "exec Admin.sp_" + tableName + "_Delete_PhanQuyen '" + nhomid+ "'";
            RICDB.DB.RunSQLNoReturn(sql, ref msg, strcnn);
            
            sql = "EXEC Admin.sp_tblNhomQuyen_Menu_Insert ";
            try
            {
