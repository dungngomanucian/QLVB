...
namespace RICTotalAdmin.Models
{
    public class ChucVuModel
    {
        ...
        {
            string sql = "exec Admin.sp_" + tblname + "_Get_List_Parent ";
            return common.RunSQLToJson(sql, ref msg);
            //return common.RunSQLToJsonManual(sql, ";MN_Parent_Name;MN_Id;MN_Ten;MN_Url;MN_Icon;MN_Thutu, active;", ref msg);
        }
        ...
        public bool Insert(dynamic row, string uid)
        {
           ...
           // string CV_Id = Guid.NewGuid().ToString();
            ...
            if (objCommon.check_code("tblChucVu", "CV_Code", row.CV_Code.ToString()))
            {
            ...
            cmd.Parameters.Add(new SqlParameter("@CV_Id", row.id.ToString()));
            //cmd.Parameters.Add(new SqlParameter("@CV_CQL_Id", row.CV_CQL_Id.ToString()));

            cmd.Parameters.Add(new SqlParameter("@CV_Code", row.CV_Code.ToString()));
            cmd.Parameters.Add(new SqlParameter("@CV_Name", row.CV_Name.ToString()));
            cmd.Parameters.Add(new SqlParameter("@CV_Status", row.active.ToString()));
            cmd.Parameters.Add(new SqlParameter("@CV_CreatedById", userId));
        ...
        public bool Update(dynamic row, string uid)
        {
            ...
            string New_Code = row.CV_Code.ToString().Trim();
            ...
            cmd.Parameters.Add(new SqlParameter("@CV_Name", row.CV_Name.ToString()));
            cmd.Parameters.Add(new SqlParameter("@CV_Status", row.active.ToString()));
            cmd.Parameters.Add(new SqlParameter("@CV_UpdatedById", userId));
